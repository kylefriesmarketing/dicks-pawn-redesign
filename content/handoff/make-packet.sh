#!/usr/bin/env bash
#
# Rebuild the ChatGPT handoff packet from tracked repo files.
#
...
# Anywhere with those two binaries will produce an identical packet.
#
# Output: dist/dicks-pawn-handoff-packet.zip  (~11 MB, 32 files)

set -euo pipefail
cd "$(dirname "$0")/../.."          # repo root
OUT=dist/packet
rm -rf "$OUT" && mkdir -p "$OUT"/{02-prompts,03-reference/store-photos,04-build,06-final-video,07-commercial}

# --- text ---------------------------------------------------------------
cp content/handoff/START-HERE.md          "$OUT/00-START-HERE.md"
cp content/video-series-bible.md          "$OUT/01-series-bible.md"
cp content/prompts/0{0,1,2,3}-*.md        "$OUT/02-prompts/"
cp content/output/README.md               "$OUT/02-prompts/04-post-production-notes.md"
...
# --- video --------------------------------------------------------------
# 720p reference cut of the delivered 1080p master: 27 MB -> 5.4 MB.
ffmpeg -y -v error -i content/output/ep01-5-myths-FINAL.mp4 \
  -vf "scale=720:1280" -c:v libx264 -crf 26 -preset slow -pix_fmt yuv420p \
  -c:a aac -b:a 112k -movflags +faststart "$OUT/06-final-video/ep01-5-myths-720p.mp4"
